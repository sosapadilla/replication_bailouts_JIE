...
INTEGER :: 	writeout = 1, simulonly = 0, maxiter = 47, runwelf = 1
! set runwelf = 1 with bailouts / 0 without
...
PARAMETER (b_num= 50, y_num = 25, tax_num = 25, transfer_num = 50, epsilon_num = 4, var_e_num = 10)
! set transfer_num = 50 with bailouts / 1 without
...
DOUBLE PRECISION :: b_grid(1:b_num), y_grid(1:y_num), default_grid(1:2),            &
    ...
    Erd(y_num), bc_grid(1:2), avg_welf_b(b_num), var_e_grid(var_e_num), d_prob(b_num,y_num)
...
PARAMETER (sigma = 2d+0, pi_number = 3.1415926535897932d+0, lambda = 0d+0, zero = 0d+0, width = 1.5d+0,     &
    ...
    epsmax = EXP(1.0d+0), bankcrisis_prob = 0.0181d+0, rw = 0.04d+0, eta=0.00d+0, psi = 0.05d+0)

! Parameters for "SMM":
DOUBLE PRECISION :: beta = 0.81232233d+0, AA = 0.27552786d+0, gov_spending = 0.14646447d+0,         &
    sig_e = 4.2570787d+0, T_max_frac = 1.0d+0
! set T_max_frac = 1.0d+0 with bailouts / 0.0d+0 without
! SMM parameters automatically adjust to recalibrated values for theta<1 and bailouts-in-default cases
...
INTEGER, DIMENSION(b_num,y_num) :: i_var_e_opt, i_var_e_opt_nb
...
DOUBLE PRECISION :: avg_loan_to_y, avg_transfer_to_y, def_prob_unconditional, def_prob_conditional, &
    ...
    bc_prob_unconditional, avg_var_e, avg_var_e_nb, avg_var_e_tranquil, avg_t_ind
...
first_mom, second_mom, std_pareto, sig_e_local !, std_dev_eshock
...
output_no_transfer, output_opt_transfer, temp_vector(var_e_num), acum_d
...
DO WHILE(convergence<0)
            ...
            do i_var = 1, var_e_num
                ...
                DO i_e_next = 1, epsilon_num ! this is looping over all future epsilon's
                    DO i_y_next = 1,y_num ! this is looping over all future y's
                        acum_w = acum_w + ( (1d+0-prob_excl_end)*value_bank_default_matrix(i_b_prime, i_y_next, i_e_next) + &
                        prob_excl_end*value_bank_matrix(i_b_prime, i_y_next, i_e_next) )*&
    ...
    ELSE
        i_var_e_opt = var_e_num
    ENDIF
   


    ! Compute expected continuation values and default probabilities
    DO i_y = 1,y_num
        i_y_global = i_y
        ...
        DO i_b = 1,b_num
            ...
            acum_d = 0d+0 ! Compute default probability
            DO j=1, epsilon_num ! this is looping over all future epsilon's
                DO i=1,y_num
                    acum_v = acum_v + value_hh_matrix(i_b, i, j)                &
                        *trans_matrix(i_y_global, i)*petran(j,i_var_e_opt(i_b,i_y))
                    acum_w = acum_w + value_bank_matrix(i_b, i, j)              &
                        *trans_matrix(i_y_global, i)*petran(j,i_var_e_opt(i_b,i_y))
                    acum_d = acum_d + default_grid(default_decision(i_b, i, j)) &
                        *trans_matrix(i_y_global, i)*petran(j,i_var_e_opt(i_b,i_y))
                END DO
            END DO
    ...
    DO i_e = 1, epsilon_num
        ...
        DO i_y = 1,y_num
            ...
            IF (indicator_bailout_default > 0.5d+0) THEN
                ...
                CALL optimize_lp(index_opt_transfer, cons_no_bc, labor_no_bc,   &
                    rate_no_bc, tax_no_bc, cons_bc, labor_bc, rate_bc, tax_bc)
        ...
        DO i_y = 1,y_num
            ...
            DO i_b = 1,b_num 
                ...
                DO i_crisis=1,2
                    ...
                    cons_banker = transfer0_matrix(i_b, i_y,i_e,i_crisis)               &
                        + b_initial - b_grid(i_b_next)*q_vector(i_b_next, i_y_global)   &
                        + loan0_matrix(i_b, i_y,i_e,i_crisis) * r0_matrix(i_b, i_y,i_e,i_crisis)
                    cons_bank0_matrix(i_b, i_y,i_e,i_crisis) = cons_banker

                    v0_matrix_new(i_b, i_y,i_e,i_crisis)  = objective_function(         &
                        consumption0_matrix(i_b, i_y,i_e,i_crisis), labor0_matrix(i_b, i_y,i_e,i_crisis))
                ...
                value_hh_repay_matrix_new(i_b, i_y,i_e) =           &
                    (1d+0-pi_global)*v0_matrix_new(i_b, i_y,i_e,1)  &
                    + pi_global *v0_matrix_new(i_b, i_y,i_e,2)
                value_bank_repay_matrix_new(i_b, i_y,i_e) =         &
                    (1d+0-pi_global)*w0_matrix_new(i_b, i_y,i_e,1)  &
                    + pi_global *w0_matrix_new(i_b, i_y,i_e,2)
                value_repay_matrix_new(i_b, i_y,i_e) =              &
                    (1d+0-pi_global)*vw0_matrix_new(i_b, i_y,i_e,1) &
                    ...
                    DO i_b = 1,b_num
                        ...
                        IF (dd > 0.5d+0) THEN
                            IF (dd*indicator_bailout_default > 0.5d+0) THEN
                                transfer_matrix(i_b, i_y, i_e, i_crisis)    &
                                    = transfer1_matrix(i_y,i_e,i_crisis)
                                guarantee_matrix(i_b, i_y, i_e, i_crisis)   &
                                    = guarantee1_matrix(i_y,i_e,i_crisis)
                            ELSE
                        ...
                        IF (writeout.EQ.1) THEN
                            WRITE(7, *)  value_matrix_new(i_b,i_y,i_e),     &
                                value_repay_matrix_new(i_b,i_y,i_e), value_default_matrix_new(i_b,i_y,i_e)
                            WRITE(8, *)  value_hh_matrix_new(i_b,i_y,i_e),  &
                                value_hh_repay_matrix_new(i_b,i_y,i_e), value_hh_default_matrix_new(i_b,i_y,i_e)
                            WRITE(9, *)  value_bank_matrix_new(i_b,i_y,i_e),&
                                value_bank_repay_matrix_new(i_b,i_y,i_e), value_bank_default_matrix_new(i_b,i_y,i_e)
                            WRITE(11, *) dd
                            ...
                            WRITE(28,*) r_matrix_new(i_b,i_y,i_e,i_crisis), &
                                r0_matrix(i_b, i_y,i_e,i_crisis), r1_matrix(i_y,i_e,i_crisis)
                            ...
                            IF (i_crisis==2) THEN
                                WRITE(46,*) transfer_matrix(i_b, i_y,i_e,i_crisis), &
                                    output_mat_no_transfer(i_b,i_y,i_e), output_mat_opt_transfer(i_b,i_y,i_e)
                            END IF

                            WRITE(47,*) guarantee_matrix(i_b,i_y,i_e,i_crisis),     &
                                guarantee0_matrix(i_b,i_y,i_e,i_crisis), guarantee1_matrix(i_y,i_e,i_crisis)
...
USE param
IMPLICIT NONE
INTEGER  i_b , i_y , index_opt_transfer, i_e, i_crisis, i
        ...
        DO i_y = 1,y_num
            ...
            IF (indicator_bailout_default > 0.5d+0) THEN
                ...
                DO i_crisis=1,2
                    ...
                    cons_bank1_matrix(i_y,i_e,i_crisis) = transfer1_matrix(i_y,i_e,i_crisis) +  &
                        loan1_matrix(i_y,i_e,i_crisis) * r1_matrix(i_y,i_e,i_crisis)

                    v1_matrix(i_y,i_e,i_crisis)  = u_fun(consumption1_matrix(i_y,i_e,i_crisis), &
                        labor1_matrix(i_y,i_e,i_crisis))
                    w1_matrix(i_y,i_e,i_crisis)  = cons_bank1_matrix(i_y,i_e,i_crisis)
            ...
            ELSE ! no bailout during default
                ...
                DO i_crisis=1,2
                    ...
                    cons_bank1_matrix(i_y,i_e,i_crisis) = r1*loan1
    ...
    DO i_e = 1,epsilon_num
        ...
        DO i_y = 1,y_num
            ...
            DO i_b = 1,b_num
                ...
                cons_bank0_matrix(i_b,i_y,i_e,i_crisis) = transfer0_matrix(i_b, i_y,i_e,i_crisis)    &
                    + b_initial + loan0_matrix(i_b, i_y,i_e,i_crisis) * r0_matrix(i_b, i_y,i_e,i_crisis)

                v0_matrix(i_b, i_y,i_e,i_crisis)  = u_fun(consumption0_matrix(i_b, i_y,i_e,i_crisis),&
                labor0_matrix(i_b, i_y,i_e,i_crisis))
                w0_matrix(i_b,i_y,i_e,i_crisis)  = cons_bank0_matrix(i_b,i_y,i_e,i_crisis)
...
501 FORMAT (I8, 35f15.10, I8, 6f15.10)
...
WRITE(5,501) num, beta, AA, sig_e, gov_spending, T_max_frac,     &
    ...
    100d+0 * avg_welf_b(i), avg_welf_b(1:20)*100.0d+0, &
    deviation, dev_q, iteration,    &
    avg_spread*100.0d+0, avg_std_spread*100.0d+0, avg_corr_spread_y, avg_rr*100.0d+0, avg_var_e, avg_var_e_nb
CLOSE (5)
...
SUBROUTINE welfar
    ...
    IF ( var_e_num>1 ) THEN
        OPEN(31, FILE='nb_files/graphs_var_e.txt')
        DO idx_y = 1,y_num
            DO idx_b = 1,b_num
                READ(31,*) i_var_e_opt_nb(idx_b, idx_y)
            END DO
        END DO
        CLOSE(31)
    ELSE
        i_var_e_opt_nb = var_e_num
    END IF
            ...
            DO idx_y = 1, y_num
                DO idx_e = 1, epsilon_num
                    ...
                    DO idx_crisis = 1,2
                        ...
                        If (cons_banker>0d+0) cons_banker = ( 1.0d+0 + ceq_grid(idx_i) )*cons_banker
                        hour = labor1_nb(idx_y, idx_e, idx_crisis)
                        evp = u_fun(con,hour)
                        evpb = cons_banker + beta_bank	&
							*(-psi *(var_e_grid(i_var_e_opt_nb(1,idx_y))-var_e_grid(var_e_num))**2)
                        DO idx_yp = 1,y_num
                            DO idx_ep = 1, epsilon_num
                                evp = evp + beta*trans_matrix(idx_y, idx_yp)*petran(idx_ep,i_var_e_opt_nb(1,idx_y)) &
								...
								evpb = evpb + beta_bank*trans_matrix(idx_y, idx_yp)		&
									*petran(idx_ep,i_var_e_opt_nb(1,idx_y)) 				&
                    ...
                    DO idx_b = 1, b_num
                        DO idx_crisis = 1,2
                            ...
                            evp = u_fun(con,hour)
                            evpb = cons_banker + beta_bank	&
								*(-psi *(var_e_grid(i_var_e_opt_nb(idx_b_next,idx_y))-var_e_grid(var_e_num))**2)
							
                            DO idx_yp = 1,y_num
                                DO idx_ep = 1, epsilon_num
                                    evp = evp + beta*trans_matrix(idx_y, idx_yp)		&
										*petran(idx_ep,i_var_e_opt_nb(idx_b_next,idx_y)) 	&
    ...
    IF (writeout.EQ.1) THEN
        ...
        OPEN (unit=9, file="graphs_welfare_b.txt", position="rewind")
        WRITE (9, *) avg_welf_b
        CLOSE (unit=9)
    ENDIF
...
DOUBLE PRECISION :: random_matrix(period_num, sample_num, 4), random_vector(1:4*sample_num*period_num), &
    b(period_num+1,sample_num), sum_defaults, sum_bc, avg_tr, avg_z
DOUBLE PRECISION, DIMENSION (period_num,sample_num) ::z, q , c, y, tax, nn, pi_firm,cons_bank,rr,loan, tax_repayment, &
wage, trans, guarantee, spread_ss, tr_y_elasticity, tr_y_multiplier, var_e_sim, var_e_nb_sim, pi_global_sim, dprob_ss 
!, welf_sim, wage_repayment, nn_repayment
...
IF (runwelf<1) THEN
    i_var_e_opt_nb = var_e_num ! Else, stored from welfare routine
ENDIF
...
DO j=1,sample_num   !SOLVE FOR SAMPLE j
   ...
   b(2,j) = b_grid(1)
   dprob_ss(1,j) = d_prob(i_b_next,i_y_previous)
   ...
   var_e_nb_sim(1,j) = var_e_grid(var_e_num)
   ...
   DO i=2,period_num
      ...
      IF (excl(i-1,j) ==2 ) THEN  !COUNTRY WAS EXCLUDED IN THE PREVIOUS PERIOD
         IF (random_matrix(i,j,4) < prob_excl_end) THEN  ! EXCLUSION ENDS THIS PERIOD
            ...
            dprob_ss(i,j) = d_prob(i_b_next,i_y_current)
            ...
            var_e_nb_sim(i,j) = var_e_grid(i_var_e_opt_nb(i_b_next,i_y_current))
         ...
         ELSE ! EXCLUSION DOES NOT END THIS PERIOD
            ...
            y(i,j) = EXP(z(i,j))*nn(i,j)**(alpha)
            dprob_ss(i,j) = prob_excl_end*d_prob(i_b_next,i_y_current)
      ...
      END IF ! CLOSES THE IF-EXCLUDED LOOPs
        ...
        IF (writeout.EQ.1) THEN
            WRITE(31,'(5F12.8,I12,6F12.8,I12,F12.8,2I12,X,2F12.8,2I12,4F12.8)') y(i,j),b(i,j), q(i,j), c(i,j),nn(i,j),&
            ...
            i_var_choice(i,j),guarantee_index(i,j), var_e_sim(i,j),var_e_nb_sim(i,j),pi_global_sim(i,j),dprob_ss(i,j)
        ENDIF
...
avg_b = SUM(b(period_start:period_num,:),                                       &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0)  &
    /DBLE(MAX(1,COUNT(trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0)))
avg_z = SUM(z(period_start:period_num,:),                                       &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0)  &
    /DBLE(MAX(1,COUNT(trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0)))
avg_y = SUM(y(period_start:period_num,:),                                       &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0)  &
    /DBLE(MAX(1,COUNT(trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0)))
...
PRINT *, 'corr(z,tr):', SUM( (trans(period_start:period_num,:)-avg_tr)*         &
    (z(period_start:period_num,:)-avg_z),                                       &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0 ) &
    /SQRT( SUM( (trans(period_start:period_num,:)-avg_tr)**2,                   &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0 ) &
    * SUM( (z(period_start:period_num,:)-avg_z)**2,                             &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0 ) )

PRINT *, 'corr(y,tr):', SUM( (trans(period_start:period_num,:)-avg_tr)*         &
    (y(period_start:period_num,:)-avg_y),                                       &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0 ) &
    /SQRT( SUM( (trans(period_start:period_num,:)-avg_tr)**2,                   &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0 ) &
    * SUM( (y(period_start:period_num,:)-avg_y)**2,                             &
    trans(period_start:period_num,:)*(2-excl(period_start:period_num,:))>0d+0 ) )
...
avg_var_e_nb = SUM(var_e_nb_sim(period_start:period_num,:),  &
    excl(period_start:period_num,:).EQ.1) /                  &
    COUNT(excl(period_start:period_num,:).EQ.1)

avg_var_e_tranquil = SUM(var_e_sim(period_start:period_num,:),   &
    (excl(period_start:period_num,:).EQ.1).AND.(dprob_ss(period_start:period_num,:)<1d-4))   &
    /COUNT((excl(period_start:period_num,:).EQ.1).AND.(dprob_ss(period_start:period_num,:)<1d-4))
...
IF (var_e_num>1) THEN
    ...
    WRITE(*,*) 'avg var e (tranquil)', avg_var_e_tranquil
    
    IF (runwelf.EQ.1) THEN 
        WRITE(*,*) 'avg var e (no bailouts)', SUM(var_e_nb_sim(period_start:period_num,:),  &
            excl(period_start:period_num,:).EQ.1)/COUNT(excl(period_start:period_num,:).EQ.1)
        WRITE(*,*) 'avg var e (no bailouts, tranquil)', SUM(var_e_nb_sim(period_start:period_num,:),    &
        (excl(period_start:period_num,:).EQ.1).AND.(dprob_ss(period_start:period_num,:)<1d-4))       &
        /COUNT((excl(period_start:period_num,:).EQ.1).AND.(dprob_ss(period_start:period_num,:)<1d-4))
    ENDIF
ENDIF
...
WRITE(*,*) 'avg var e (no bailouts)', avg_var_e_nb

IF (runwelf.EQ.1) THEN
    i = MINVAL(MINLOC(ABS(b_grid-avg_b)))
    WRITE(*,*) 'avg debt', avg_b
