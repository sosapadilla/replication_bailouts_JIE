! Code for extensions after AEJM: bailout announcement and moral hazard.
module param
IMPLICIT NONE

INTEGER :: 	writeout = 1, simulonly = 0, maxiter = 48, runwelf = 0
...
i_b_global, i_default_global, i_b_next,  i_b_nodil, iteration,  tax_num, transfer_num, epsilon_num, var_e_num
PARAMETER (b_num= 50, y_num =25, tax_num=25, transfer_num = 1, epsilon_num = 4, var_e_num=1)
...
DOUBLE PRECISION :: b_grid(1:b_num), y_grid(1:y_num), default_grid(1:2),            &
    ...
    cdfe(1:epsilon_num), petran(epsilon_num, var_e_num), cum_petran(epsilon_num, var_e_num),          &
    Erd(y_num), bc_grid(1:2), avg_welf_b(b_num), var_e_grid(var_e_num)

DOUBLE PRECISION :: sigma, b_inf, b_sup, y_inf, y_sup, eps_inf, eps_sup, std_y, pi_number, lambda,  &
    zero, width, prob_excl_end, pi, coupon, delta_bond, theta, tax_inf, tax_sup, k, mean_y, rw,     &
...
PARAMETER (sigma = 2d+0, pi_number = 3.1415926535897932d+0, lambda = 0d+0, zero = 0d+0, width = 1.5d+0,     &
    prob_excl_end = 0.5d+0, delta_bond = 1.0d+0, theta=1d+0, k=1d+0, mean_y = 0d+0, epsmin = 1.0d+0,        &
    epsmax = EXP(1.0d+0), bankcrisis_prob = 0.0181d+0, rw = 0.04d+0, eta=0.5d+0, psi = 1000000.00d+0)

! Parameters for "SMM":
DOUBLE PRECISION :: beta = 0.81232233d+0, AA = 0.27552786d+0, gov_spending = 0.14646447d+0,         &
    sig_e = 4.2570787d+0, T_max_frac = 0.0d+0
...
!Latest version has m==0. It used to be m = 0.1357 (target (inv+nx)/y = 16.52%).
...
INTEGER, DIMENSION(b_num,y_num) :: i_var_e_opt

INTEGER, DIMENSION(b_num, y_num, epsilon_num) :: index_opt_tr_matrix0
INTEGER, DIMENSION(y_num, epsilon_num) :: index_opt_tr_matrix1
...
INTEGER, DIMENSION(b_num,y_num, epsilon_num) ::  default_decision, default_decision_new
...
DOUBLE PRECISION :: avg_loan_to_y, avg_transfer_to_y, def_prob_unconditional, def_prob_conditional, &
    avg_b_to_y, avg_g_to_y, avg_std_log_y, T_max, avg_welf, avg_spread, avg_std_spread,             &
    avg_corr_spread_y, avg_rr, avg_log_y, avg_y, avg_spread_bc, avg_std_spread_bc, avg_b,&
    bc_prob_unconditional, avg_var_e, avg_t_ind, avg_bailout_to_y_unconditional
...
INTEGER FUNCTION multpl(i,m_local)
IMPLICIT NONE
INTEGER, INTENT(IN) :: i,m_local
REAL*8, PARAMETER :: toler = 1.0D-12
REAL*8 :: remain

remain = DBLE(REAL(i))/DBLE(REAL(m_local))-i/m_local
...
if ( var_e_num >1 ) then
    CALL creategrid(var_e_grid, var_e_num, 0.2d+0, 1.2d+0/2d+0, 1.00d+0, 1.0d+0, 1.0d+0)
else
    var_e_grid=1.0d+0    
end if

PRINT *, "var e grid", var_e_grid
...
! PRINT *, "first moment", first_mom
! PRINT *, "second moment", second_mom
! PRINT *, "std_dev of untransformed grid", std_pareto
...
ELOOP:  DO i =1,epsilon_num-1
    ...
    DO i_sigma_e = 1,var_e_num
        sig_e_local =  sig_e * var_e_grid(i_sigma_e)

        DO i =1,epsilon_num-1
            ! CALL pareto_cdf(epsmin, epsmax, sig_e_local, eshock(i) + 0.5d+0*std_pareto, cdfe(i))
            CALL pareto_cdf(epsmin, epsmax, sig_e_local, (eshock(i) + eshock(i+1))/2d+0, cdfe(i))
            IF (i.EQ.1) THEN
               petran(i,i_sigma_e) = cdfe(i)
            ELSE
               petran(i,i_sigma_e) = cdfe(i) - cdfe(i-1)
            ENDIF
        END DO
        petran(epsilon_num,i_sigma_e) = 1d+0 - cdfe(epsilon_num-1)
    END DO
   ...
   PRINT *, "pe tran: ", petran(i,i_sigma_e), "for i ", i, " and ivar ",i_sigma_e
        end do
    end do

   eshock = LOG(eshock)
   PRINT *, "transformed e shocks: ", eshock
   PRINT *, "E(e shock): ", SUM(eshock*petran(:,var_e_num))
   ...
   ! PRINT *, "std_dev of transformed grid", std_dev_eshock
    ...
    DO i_sigma_e = 1,var_e_num
        cum_petran(1,i_sigma_e) = petran(1,i_sigma_e)
           DO j=2,epsilon_num
               cum_petran(j,i_sigma_e) = cum_petran(j-1,i_sigma_e) + petran(j,i_sigma_e)
           END DO
        cum_petran(epsilon_num,i_sigma_e) = 1.0d+0
    END DO
...
IF(indicator_lumpsum > 0.5d+0) THEN
   tax_lp_and_default = gov_spending + (1d+0 - d) * b_initial + transfer_effective !In this case, the tax_lp_and_default doesn't affect N nor r, so I can compute here.
...
ELSE
   ...
   IF (rr==r_min) THEN ! meaning the intersection of demand an supply happens at the horizontal part of the demand and:
                       ! it is not longer true that tax_base= AA/gamma_firm. Then I have to solve for this special case.
                       ! the loan will be demand determined:
...
IF(indicator_lumpsum > 0.5d+0) THEN
   nodef_part = b_initial+transfer_effective - b_grid(i_b_next) * q_vector(i_b_next,i_y_global)
...
ELSE
   ...
   IF (rr==r_min) THEN ! meaning the intersection of demand an supply happens at the horizontal part of the demand and:
      ...
      ELSE
         ...
         !exit flag is 1, THEN something is not right--
         !WRITE (6,*)'b',i_b_global,'y',i_y_global
         !pause
...
DOUBLE PRECISION :: y_next, m_next, exp_m, exp_q, prob_crisis(2), pi_local
...
DO i_crisis_next = 1,2
   DO i_e_next = 1,epsilon_num
      DO i_y_next=1,y_num
         pi_local = pi * (1d+0 - eta * transfer_grid(index_opt_tr_matrix0(i_b,i_y_next,i_e_next)))
         prob_crisis(1)=1d+0-pi_local
         prob_crisis(2)=pi_local
         y_next = y_grid(i_y_next)
         m_next = beta_bank
         exp_m   = exp_m + m_next * coupon * (1d+0 - default_grid(default_decision(i_b, i_y_next, i_e_next))) * &
         (1d+0 + r_matrix(i_b, i_y_next, i_e_next, i_crisis_next)) *&
         trans_matrix(i_y, i_y_next)*petran(i_e_next,i_var_e_opt(i_b,i_y))*prob_crisis(i_crisis_next)
      
         exp_q   = exp_q + (1d+0 - delta_bond) * m_next * q_matrix_nodef(i_b, i_y_next, i_e_next, i_crisis_next) * &
         (1d+0 - default_grid(default_decision(i_b, i_y_next, i_e_next))) * &
         trans_matrix(i_y, i_y_next) *petran(i_e_next,i_var_e_opt(i_b,i_y))*prob_crisis(i_crisis_next)
...
DO i=1,transfer_num
   ...
   transfer_global   = min(AA - A_worst_case, T_max) * transfer_grid(i)
    pi_global = pi * (1d+0 - eta* transfer_grid(i))
   DO j=1,2
   ...
   END DO
   vector(i) = (1d+0 -pi_global) *  welfare(i,1) + pi_global *  welfare(i,2)
END DO
...
IF (exit_flag<0.5d+0) THEN
   ...
   ! REMEMBER THIS STAGE SHOULD NEVER OCURR!!!... SO IF I GET THE MESSAGE, SOMETHING HAS TO BE WRONG BEFORE
...
DO i_t=1,transfer_num
   transfer_global   = min(AA - A_worst_case, T_max) * transfer_grid(i_t)    
    pi_global = pi * (1d+0 - eta* transfer_grid(i_t))
...
DO j=1,2
      ...
      DO i_b=1,b_num
         i_b_next = i_b
         ! call optimal_var_e_matrix(i_y_global,i_b_next) --> make it a global variable
   ...
   vector_outside(i_t) = (1d+0 -pi_global) *  welfare(bp_choice(i_t,1),i_t,1) &
+ pi_global * welfare(bp_choice(i_t,2),i_t,2)
   output_outside(i_t) = (1d+0 -pi_global) *  output_vector(bp_choice(i_t,1),i_t,1) &
+ pi_global * output_vector(bp_choice(i_t,2),i_t,2)
END DO
         
    ! OPEN (46, FILE='ouput_trcand.txt')
...
i_b_prime, i_e_next, i_y_next, i_var, temp_index(1)
...
output_no_transfer, output_opt_transfer, temp_vector(var_e_num)
...
DO WHILE(convergence<0)
   ...
   i_b_prime = 1
   do i_y = 1, y_num    
        do i_var = 1, var_e_num
            acum_w = -(psi *(var_e_grid(i_var)-var_e_grid(var_e_num))**2)
            DO i_e_next = 1, epsilon_num ! this is looping over all future epsilon's
                DO i_y_next = 1,y_num ! this is looping over all future y's
                    acum_w = acum_w + value_bank_default_matrix(i_b_prime, i_y_next, i_e_next) *&
                     trans_matrix(i_y, i_y_next)*petran(i_e_next,i_var)
                end do
            end do
            temp_vector(i_var) = acum_w
        end do
        temp_index = maxloc(temp_vector) 
        i_var_e_opt(i_b_prime,i_y) = temp_index(1)
    end do

    do i_b_prime = 2, b_num
        do i_y = 1, y_num        
            do i_var = 1, var_e_num
                acum_w = -(psi *(var_e_grid(i_var)-var_e_grid(var_e_num))**2)
                DO i_e_next = 1, epsilon_num ! this is looping over all future epsilon's
                    DO i_y_next = 1,y_num ! this is looping over all future y's
                        acum_w = acum_w + value_bank_matrix(i_b_prime, i_y_next, i_e_next) *&
                         trans_matrix(i_y, i_y_next)*petran(i_e_next,i_var)
                    end do
                end do
                temp_vector(i_var) = acum_w
        ...
        ! max over var_e E_{y',e'|y} {- cost + value_bank_matrix }
        ! store optimal_var_e --> optimal_var_e_matrix(y,b')
        ! We need to do this for repay and default (or more precisely, with and without access)
        ! 
        ! We also need to compute the expectation of re-entering tomorrow, if you ended the period w/o access:
        ! (prob_excl_end* exp_w_next + (1d+0-prob_excl_end)*exp_w_next_excl)
        ! 
        ! Note: we'll assume that if the country is choosing b'=0 then it's because it is excluded today (and therefore we need to 
        ! account for the prob of reentry tomorrow), otherwise it has access today.
   ...
   DO i_y = 1,y_num
      ...
      acum_v_d = 0d+0
      acum_w_d = -(psi *(var_e_grid(i_var_e_opt(1,i_y))-var_e_grid(var_e_num))**2)
      DO j=1, epsilon_num ! this is looping over all future epsilon's
         DO i=1,y_num ! this is looping over all future y's
            acum_v_d = acum_v_d + value_hh_default_matrix(1,i,j) * trans_matrix(i_y_global, i) * petran(j,i_var_e_opt(1,i_y))
            acum_w_d = acum_w_d + value_bank_default_matrix(1,i,j) * trans_matrix(i_y_global, i)* petran(j,i_var_e_opt(1,i_y))
         END DO
      END DO
      ...
      DO i_b = 1,b_num
         acum_v = 0d+0
         acum_w = -(psi *(var_e_grid(i_var_e_opt(i_b,i_y))-var_e_grid(var_e_num))**2)
         DO j=1, epsilon_num ! this is looping over all future epsilon's
            DO i=1,y_num
                acum_v = acum_v + value_hh_matrix(i_b, i, j) * trans_matrix(i_y_global, i) *petran(j,i_var_e_opt(i_b,i_y))
                acum_w = acum_w + value_bank_matrix(i_b, i, j) * trans_matrix(i_y_global, i)*petran(j,i_var_e_opt(i_b,i_y))
            END DO
         END DO
   ...
   DO i_e = 1, epsilon_num
      ...
      DO i_y = 1,y_num
         ...
         IF (indicator_bailout_default > 0.5d+0) then
            ...
            pi_global = pi * (1d+0 - eta * transfer_grid(index_opt_transfer))   
            index_opt_tr_matrix1 (i_y,i_e) =  index_opt_transfer
            ...
            DO i_crisis=1,2               
                ...
                transfer1_matrix(i_y,i_e,i_crisis) = (1d+0 - bc) * 0d+0 +   &
                    bc * min(AA-A_worst_case,T_max)*transfer_grid(index_opt_transfer)   
                guarantee1_matrix(i_y,i_e,i_crisis) =                       &
                    min(AA-A_worst_case,T_max)*transfer_grid(index_opt_transfer)   
         ...
         ELSE ! no bailout during default
            ...
            pi_global = pi
            index_opt_tr_matrix1 (i_y,i_e) =  1 ! No bailout
         ...
         value_hh_default_matrix_new(:, i_y, i_e) = (1d+0-pi_global)*v1_matrix_new(i_y,i_e,1) &
            + pi_global *v1_matrix_new( i_y,i_e,2)
         value_bank_default_matrix_new(:, i_y, i_e) = (1d+0-pi_global)*w1_matrix_new(i_y,i_e,1)&
            + pi_global *w1_matrix_new( i_y,i_e,2)
         value_default_matrix_new(:, i_y, i_e) = (1d+0-pi_global)*vw1_matrix_new(i_y,i_e,1) &
            + pi_global *vw1_matrix_new( i_y,i_e,2)
   ...
   DO i_e = 1, epsilon_num
      ...
      DO i_y = 1,y_num
         ...
         DO i_b = 1,b_num 
            ...
            pi_global = pi * (1d+0 - eta * transfer_grid(i_opt_t))    
            index_opt_tr_matrix0(i_b,i_y,i_e) = i_opt_t
            ...
            DO i_crisis=1,2
               ...
               transfer0_matrix(i_b, i_y,i_e,i_crisis) = (1d+0 - bc) * 0d+0 + bc * min(AA - A_worst_case, T_max)*&
               transfer_grid(i_opt_t)          
               guarantee0_matrix(i_b, i_y,i_e,i_crisis) = min(AA - A_worst_case, T_max)*transfer_grid(i_opt_t)
            ...
            value_hh_repay_matrix_new(i_b, i_y,i_e) = (1d+0-pi_global)*v0_matrix_new(i_b, i_y,i_e,1) &
+ pi_global *v0_matrix_new(i_b, i_y,i_e,2)
            value_bank_repay_matrix_new(i_b, i_y,i_e) = (1d+0-pi_global)*w0_matrix_new(i_b, i_y,i_e,1) &
+ pi_global *w0_matrix_new(i_b, i_y,i_e,2)
            value_repay_matrix_new(i_b, i_y,i_e) = (1d+0-pi_global)*vw0_matrix_new(i_b, i_y,i_e,1)&
+ pi_global *vw0_matrix_new(i_b, i_y,i_e,2)
   ...
   r_matrix = r_matrix_new
    ...
    IF (deviation < criteria .OR. iteration >= maxiter) THEN
        IF (writeout.EQ.1) THEN
            ...
            OPEN (48, FILE='graphs_guarantees_index.txt')
            OPEN (49, FILE='graphs_var_e.txt')
         ...
         DO i_y = 1,y_num
            ...
            DO i_b = 1,b_num
                ...
                IF (writeout.EQ.1) THEN
                    ...
                    WRITE(48,*) index_opt_tr_matrix0(i_b,i_y,i_e), index_opt_tr_matrix1(i_b,i_y) 
					WRITE(49,*) i_var_e_opt(i_b,i_y)
...
!ENDIF
ELSE
    IF (theta<0.99d+0) THEN
        IF (theta>0.89d+0) THEN
            beta = 0.79597171d+0
            AA = 0.28257079d+0
            gov_spending = 0.147625d+0
            sig_e = 4.2d+0
        ELSEIF (theta>0.79d+0) THEN
            beta = 0.85354648d+0
            AA = 0.28257079d+0
            gov_spending = 0.147625d+0
            sig_e = 4.456362d+0
        ELSEIF (theta>0.69d+0) THEN
            beta = 0.87085806d+0
            AA = 0.27d+0
            gov_spending = 0.14374669d+0
            sig_e = 3.0d+0
        ELSEIF (theta>0.59d+0) THEN
            beta = 0.87085806d+0
            AA = 0.27d+0
            gov_spending = 0.14374669d+0
            sig_e = 3.0d+0
        ELSEIF (theta>0.49d+0) THEN
            beta = 0.81845299d+0
            AA = 0.31265986d+0
            gov_spending = 0.145d+0
            sig_e = 3.7528921d+0
        ENDIF
    END IF
ENDIF
...
IF (indicator_external < 0.5d+0) THEN
    ...
    DO i_e = 1,epsilon_num
        ...
        DO i_y = 1,y_num
            ...
            ELSE ! no bailout during default
                pi_global = pi
                index_opt_tr_matrix1 (i_y,i_e) =  1
            ...
            value_hh_default_matrix(:, i_y, i_e) = (1d+0-pi_global)*v1_matrix(i_y,i_e,1) + pi_global *v1_matrix( i_y,i_e,2)
            value_bank_default_matrix(:, i_y, i_e) = (1d+0-pi_global)*w1_matrix(i_y,i_e,1) + pi_global *w1_matrix( i_y,i_e,2)
            value_default_matrix(:, i_y, i_e) = (1d+0-pi_global)*vw1_matrix(i_y,i_e,1) + pi_global *vw1_matrix( i_y,i_e,2)
   ...
   DO i_e = 1,epsilon_num
      ...
      DO i_y = 1,y_num
         ...
         CALL optimize_lp(index_opt_transfer, cons_no_bc, labor_no_bc, rate_no_bc, tax_no_bc, cons_bc, labor_bc, rate_bc, tax_bc)
         pi_global = pi * (1d+0 - eta * transfer_grid(index_opt_transfer))    
         index_opt_tr_matrix0(i_b,i_y,i_e) = index_opt_transfer
            ...
            END DO
            value_hh_repay_matrix(i_b, i_y,i_e) = (1d+0-pi_global)*v0_matrix(i_b, i_y,i_e,1) &
+ pi_global *v0_matrix(i_b, i_y,i_e,2)
            value_bank_repay_matrix(i_b, i_y,i_e) = (1d+0-pi_global)*w0_matrix(i_b, i_y,i_e,1) &
+ pi_global *w0_matrix(i_b, i_y,i_e,2)
            value_repay_matrix(i_b, i_y,i_e) = (1d+0-pi_global)*vw0_matrix(i_b, i_y,i_e,1)&
+ pi_global *vw0_matrix(i_b, i_y,i_e,2)
...
501 FORMAT (I8, 35f15.10, I8, 8f15.10)
...
WRITE(5,501) num, beta, AA, sig_e, gov_spending, T_max_frac,     &
    ...
    avg_bailout_to_y_unconditional*100.0d+0, bc_prob_unconditional*100.0d+0, &
    avg_transfer_to_y*100.0d+0, avg_welf_b(i)*100.0d+0
CLOSE (5)
...
SUBROUTINE welfar
    ...
    DOUBLE PRECISION :: con, hour, evp, evp1, evpb, evpb1, dum, cons_banker
    DOUBLE PRECISION, DIMENSION (:,:,:,:), ALLOCATABLE :: vhati,vhati_old, whati, whati_old, vwhati
    ...
    DOUBLE PRECISION, DIMENSION (y_num,epsilon_num) :: vdeftemp, wdeftemp
	...
	ALLOCATE(whati(b_num,y_num,epsilon_num,ceq_num))
    ALLOCATE(whati_old(b_num,y_num,epsilon_num,ceq_num))
    ALLOCATE(vwhati(b_num,y_num,epsilon_num,ceq_num))
    ...
    DO idx_b = 1,b_num
        ...
        READ(30,*) bank_nb(idx_b,idx_y, idx_e, idx_crisis), bank0_nb(idx_b,idx_y, idx_e, idx_crisis),   &
            ...
            ! Need to read optimal_choice_of_var_e (function of y_today, and b_nxt)
    END DO
    END DO
    END DO
    END DO
    ...
    DO idx_i = 1, ceq_num
            ...
            DO idx_y = 1, y_num
                DO idx_e = 1, epsilon_num
                    ...
                    DO idx_crisis = 1,2
                        ...
                        cons_banker = ( 1.0d+0 + ceq_grid(idx_i) )*bank1_nb(idx_y, idx_e, idx_crisis)
                        hour = labor1_nb(idx_y, idx_e, idx_crisis)
                        evp = u_fun(con,hour)
                        evpb = cons_banker + beta_bank	&
							*(-psi *(var_e_grid(i_var_e_opt(1,idx_y))-var_e_grid(var_e_num))**2)
                        DO idx_yp = 1,y_num
                            DO idx_ep = 1, epsilon_num
                                evp = evp + beta*trans_matrix(idx_y, idx_yp)*petran(idx_ep,i_var_e_opt(1,idx_y)) &
                                    *(prob_excl_end*vhati_old(1,idx_yp, idx_ep, idx_i)	&
                                    +(1.0d+0-prob_excl_end)*vdeftemp(idx_yp, idx_ep))
								evpb = evpb + beta_bank*trans_matrix(idx_y, idx_yp)		&
									*petran(idx_ep,i_var_e_opt(1,idx_y)) 				&
                                    *(prob_excl_end*whati_old(1,idx_yp, idx_ep, idx_i)	&
                                    +(1.0d+0-prob_excl_end)*wdeftemp(idx_yp, idx_ep))
							END DO
                        END DO
                        ...
                        END IF
                    END DO
                    vdeftemp(idx_y, idx_e) = (1.0d+0 - pi_global)*evp1 + pi_global*evp
                    wdeftemp(idx_y, idx_e) = (1.0d+0 - pi_global)*evpb1 + pi_global*evpb
                    
                    ! Repay
                    DO idx_b = 1, b_num
                        DO idx_crisis = 1,2
                            con = ( 1.0d+0 + ceq_grid(idx_i) )*cons0_nb(idx_b, idx_y, idx_e, idx_crisis)
                            cons_banker = ( 1.0d+0 + ceq_grid(idx_i) )*bank0_nb(idx_b, idx_y, idx_e, idx_crisis)
                            hour = labor0_nb(idx_b, idx_y, idx_e, idx_crisis)
                            idx_b_next = bnext0_nb(idx_b, idx_y, idx_e, idx_crisis)
                        
                            evp = u_fun(con,hour)
                            evpb = cons_banker + beta_bank	&
								*(-psi *(var_e_grid(i_var_e_opt(idx_b_next,idx_y))-var_e_grid(var_e_num))**2)
							
                            DO idx_yp = 1,y_num
                                DO idx_ep = 1, epsilon_num
                                    evp = evp + beta*trans_matrix(idx_y, idx_yp)		&
										*petran(idx_ep,i_var_e_opt(idx_b_next,idx_y)) 	&
                                        *vhati_old(idx_b_next,idx_yp, idx_ep, idx_i)
                                    evpb = evpb + beta_bank*trans_matrix(idx_y, idx_yp)	&
										*petran(idx_ep,i_var_e_opt(idx_b_next,idx_y)) 	&
                                        *whati_old(idx_b_next,idx_yp, idx_ep, idx_i)
                                END DO
                            END DO
                            IF (idx_crisis.EQ.1) THEN
                                evp1 = evp
                        ...
                        IF (def_nb(idx_b, idx_y, idx_e)>0.5d+0) THEN
                            ...
                            whati(idx_b, idx_y, idx_e, idx_i) = wdeftemp(idx_y, idx_e)
                        ELSE
                            vhati(idx_b, idx_y, idx_e, idx_i) = (1.0d+0 - pi_global)*evp1 + pi_global*evp
                            whati(idx_b, idx_y, idx_e, idx_i) = (1.0d+0 - pi_global)*evpb1 + pi_global*evpb
                        ENDIF
        ...
        END DO
    END DO
	vwhati = theta*vhati + (1.0d+0-theta)*whati
    ...
    DO idx_b=1,b_num
        DO idx_y = 1,y_num
            DO idx_e = 1, epsilon_num
                IF ((vwhati(idx_b, idx_y, idx_e, 1)-value_matrix(idx_b, idx_y, idx_e)) &
                    *(vwhati(idx_b, idx_y, idx_e, ceq_num)-value_matrix(idx_b, idx_y, idx_e))<0.0d+0) THEN
    ...
    DO idx_b = 1,b_num
        DO idx_y = 1,y_num
            DO idx_e = 1, epsilon_num
                avg_welf_b(idx_b) = avg_welf_b(idx_b) + welfares(idx_b, idx_y, idx_e)*Erd(idx_y)*&
                petran(idx_e,var_e_num)
                ! Note, here it makes sense to compute this ex-ante expectation using the largest possible variance for e-shocks
            END DO
        END DO
    END DO
    ...
    vc = interp1(ceq_grid, vwhati(idx_b, idx_y, idx_e, :), ceq_num, ceq)
...
INTEGER :: period_num, period_start, i,j,sample_num, default_num, i_y_previous, i_y_current, i_b_current, &
    i_e_current, i_crisis_current, counter_def_conditional
PARAMETER (sample_num =500, period_num=1501, period_start=51)
INTEGER, DIMENSION (period_num,sample_num) :: dd, excl, bc_crisis, y_dev, y_trend, i_var_choice, guarantee_index
DOUBLE PRECISION :: random_matrix(period_num, sample_num, 4), random_vector(1:4*sample_num*period_num), &
    b(period_num+1,sample_num), sum_defaults, sum_bc, avg_tr
DOUBLE PRECISION, DIMENSION (period_num,sample_num) ::z, q , c, y, tax, nn, pi_firm,cons_bank,rr,loan, tax_repayment, &
wage, trans, guarantee, spread_ss, tr_y_elasticity, tr_y_multiplier, var_e_sim, pi_global_sim !, welf_sim, wage_repayment, nn_repayment
DOUBLE PRECISION, EXTERNAL :: q_fun, objective_function, r_fun
...
DO j=1,sample_num   !SOLVE FOR SAMPLE j
   ...
   i_var_choice(1,j) = var_e_num
   var_e_sim(1,j) = var_e_grid(i_var_choice(1,j))
   
   guarantee_index(1,j)=1 !Initialize this to one
   ...
   DO i=2,period_num
      ...
      i_e_current = 1      
      DO WHILE(cum_petran(i_e_current, i_var_choice(i-1,j)) < random_matrix(i,j,2))
      ...
      IF (excl(i-1,j) ==2 ) THEN  !COUNTRY WAS EXCLUDED IN THE PREVIOUS PERIOD
         IF (random_matrix(i,j,4) < prob_excl_end) THEN  ! EXCLUSION ENDS THIS PERIOD
            ...
            guarantee_index(i,j)= index_opt_tr_matrix0(i_b_current,i_y_current,i_e_current)
            pi_global= pi * (1d+0 - eta *transfer_grid(guarantee_index(i,j)))
            ! Need to define the relevant pi_global, which depends on the Transfer policy
!            IF (AA>A_worst_case .and. T_max>0 ) THEN
!            ! pi_global = pi * (1d+0 - eta * (guarantee0_matrix(i_b_current,i_y_current,i_e_current,1))/&
!            !   min(AA - A_worst_case, T_max))
!                pi_global= pi * (1d+0 - eta *transfer_grid(guarantee_index(i,j)))
!            ELSE
!            pi_global = pi
!            ENDIF
            pi_global_sim(i,j) = pi_global


            IF (random_matrix(i,j,3) < pi_global) THEN
            ...
            i_b_next = b_next_matrix(i_b_current, i_y_current, i_e_current, i_crisis_current) !! THIS I HAVE BEFORE, BUT IT SHOULD BE:
            ...
            cons_bank(i,j) = trans(i,j) + loan(i,j) * rr(i,j) + b_initial -  b(i+1,j)*q(i,j)
            ...
            i_var_choice(i,j) = i_var_e_opt(i_b_next,i_y_current)
            var_e_sim(i,j) = var_e_grid(i_var_choice(i,j))
         ...
         ELSE ! EXCLUSION DOES NOT END THIS PERIOD
            ...
            ! Need to define the relevant pi_global, which depends on the Transfer policy
            IF (indicator_bailout_default > 0.5d+0) THEN
                guarantee_index(i,j)= index_opt_tr_matrix1(i_y_current,i_e_current)
                pi_global = pi * (1d+0 - eta * transfer_grid(guarantee_index(i,j)))
                !IF (AA>A_worst_case  .and. T_max>0 ) THEN
                !!pi_global = pi * (1d+0 - eta * (guarantee1_matrix(i_y_current,i_e_current,1))/&
                !!          min(AA - A_worst_case, T_max))
                !    pi_global = pi * (1d+0 - eta * transfer_grid(guarantee_index(i,j)))
                !ELSE
                !    pi_global = pi
            ...
            b(i+1,j) = b_grid(i_b_next)
            q(i,j) = q_fun(i_b_next, i_y_current)  !0.0d+0 ! !q_paid
            ...
            c(i,j) = consumption1_matrix(i_y_current, i_e_current, i_crisis_current)!y(i,j) -gov_spending- cons_bank(i,j)
            wage(i,j)= EXP(z(i,j))*alpha*nn(i,j)**(alpha-1d+0)/(1d+0 + gamma_firm*rr(i,j))
...
!                ENDIF
            ...
            b(i+1,j) = b_grid(i_b_next)
            q(i,j) = q_fun(i_b_next, i_y_current) !0d+0!  !q_paid
            spread_ss(i,j) = 999d+0
            tax(i,j) = tax1_matrix(i_y_current, i_e_current, i_crisis_current)
            nn(i,j) =labor1_matrix(i_y_current, i_e_current, i_crisis_current)
            y(i,j) = EXP(z(i,j))*nn(i,j)**(alpha)
...
!            ENDIF
         ...
         END IF
      END IF ! CLOSES THE IF-EXCLUDED LOOPs

        IF (writeout.EQ.1) THEN
            WRITE(31,'(5F12.8,I12,6F12.8,I12,F12.8,2I12,X,2F12.8,2I12,2F12.8)') y(i,j),b(i,j), q(i,j), c(i,j),nn(i,j),&
            dd(i,j), rr(i,j), tax(i,j),pi_firm(i,j), cons_bank(i,j),loan(i,j),tax_repayment(i,j),excl(i,j), trans(i,j),&
            i_crisis_current-1, i_e_current, spread_ss(i,j), guarantee(i,j),&
            i_var_choice(i,j),guarantee_index(i,j), var_e_sim(i,j),pi_global_sim(i,j)
        ENDIF
...
IF (writeout.EQ.1) THEN
    ...
    CLOSE (unit=9)
    OPEN (unit=9, file="graphs_simulation_zshocks.txt", position="rewind")
    WRITE (9, *) z
    CLOSE (unit=9)
END IF
...
bc_prob_unconditional = sum_bc/((period_num-period_start+1)*sample_num)

WRITE(*,*) 'BC prob unconditional (%)', 100d+0 * bc_prob_unconditional
WRITE(*,*) 'def prob unconditional (%)', 100d+0 * def_prob_unconditional
WRITE(*,*) 'def prob conditional on BC (%)', 100d+0 * def_prob_conditional

!PRINT *, 'output-transfer elasticity (percent):',  100.0d+0*    &
!    SUM(tr_y_elasticity(period_start:period_num,:), tr_y_elasticity(period_start:period_num,:)>-999d+0) &
!    /DBLE(MAX(1,COUNT(tr_y_elasticity(period_start:period_num,:)>-999d+0)))
...
!index = MAXVAL(MINLOC(ABS(b_grid-avg_b)))

!avg_welf = zero
!DO idx_y = 1,y_num
!    DO idx_e = 1, epsilon_num
!        avg_welf = avg_welf + welfares(index, idx_y, idx_e)*Erd(idx_y)*petran(idx_e)
!    END DO
!END DO
!WRITE(*,*) 'welfare gain rel. to no bailout (percent, average debt)', 100d+0 * avg_welf
...
avg_bailout_to_y_unconditional = SUM(guarantee(period_start:period_num,:)/y(period_start:period_num,:),  &
    excl(period_start:period_num,:).EQ.1)/COUNT(excl(period_start:period_num,:).EQ.1)
...
avg_rr = SUM( rr(period_start:period_num,:), excl(period_start:period_num,:).EQ.1)/ &
    ...
    avg_var_e = SUM(var_e_sim(period_start:period_num,:),   &
    excl(period_start:period_num,:).EQ.1)/COUNT(excl(period_start:period_num,:).EQ.1)

    avg_t_ind = SUM(guarantee_index(period_start:period_num,:),    &
    excl(period_start:period_num,:).EQ.1)/COUNT(excl(period_start:period_num,:).EQ.1)
    
PRINT *, '<< Simulated moments (conditional on no exclusion) >>'
WRITE(*,*) 'avg t index', avg_t_ind
WRITE(*,*) 'avg var e (%)', avg_var_e
WRITE(*,*) 'avg var e (conditional on BC,%)', SUM( var_e_sim(period_start:period_num,:), &
excl(period_start:period_num,:)*bc_crisis(period_start:period_num,:).EQ.1)/ &
COUNT(excl(period_start:period_num,:)*bc_crisis(period_start:period_num,:).EQ.1)
...
!!! << No exclusion for 50 consecutive periods >> !!!

!sample_length = 50
!counter_no_def = 0
!DO j=1,sample_num  
!      nodef_check = SUM(excl(period_num-sample_length+1:period_num,j)-1)
!      IF (nodef_check == 0) THEN
!         counter_no_def=counter_no_def +1                    
!      END IF         
!END DO

!PRINT *, 'Allocating arrays'
!ALLOCATE(y_ts(sample_length,counter_no_def))
!ALLOCATE(q_ts(sample_length,counter_no_def))
!ALLOCATE(b_ts(sample_length,counter_no_def))
!ALLOCATE(spread_ts(sample_length,counter_no_def))
!ALLOCATE(rr_ts(sample_length,counter_no_def))
!ALLOCATE(loan_ts(sample_length,counter_no_def))
!ALLOCATE(transfer_ts(sample_length,counter_no_def))
!ALLOCATE(loan_to_y(sample_length,counter_no_def))
!ALLOCATE(g_to_y(sample_length,counter_no_def))
!ALLOCATE(b_to_y(sample_length,counter_no_def))
!
!ALLOCATE(std_log_y(counter_no_def))
!ALLOCATE(mean_loan_to_y(counter_no_def))
!ALLOCATE(mean_g_to_y(counter_no_def))
!ALLOCATE(mean_b_to_y(counter_no_def))
!ALLOCATE(mean_spread(counter_no_def))
!ALLOCATE(std_spread(counter_no_def))
!ALLOCATE(mean_output(counter_no_def))
!ALLOCATE(mean_log_output(counter_no_def))
!ALLOCATE(std_output(counter_no_def))
!ALLOCATE(corr_spread_y(counter_no_def))
!ALLOCATE(mean_rr(counter_no_def))
...
!      nodef_check = SUM(excl(period_num-sample_length+1:period_num,j)-1)
!      IF (nodef_check == 0) THEN
!         DO i=period_num-sample_length+1,period_num
!            IF(bc_crisis(i,j)==1) THEN
!            counter_bc_no_def=counter_bc_no_def +1                    
!            END IF         
!         END DO
!      END IF      
!END DO

!ALLOCATE(transfer_to_y(counter_bc_no_def))
!
...
!   IF (nodef_check == 0) THEN
!        index = index+1
!        y_ts(1:sample_length, index) = y(period_num-sample_length+1:period_num,j)
!        q_ts(1:sample_length, index) = q(period_num-sample_length+1:period_num,j)
!        loan_ts(1:sample_length, index) = loan(period_num-sample_length+1:period_num,j)
!        b_ts(1:sample_length, index) = b(period_num-sample_length+1:period_num,j)
!        spread_ts(1:sample_length, index) = spread_ss(period_num-sample_length+1:period_num,j)
!        rr_ts(1:sample_length, index) = rr(period_num-sample_length+1:period_num,j)      
!        DO i=period_num-sample_length+1,period_num
!            IF(bc_crisis(i,j)==1) THEN
!            index_2=index_2 +1
!            transfer_to_y(index_2)=trans(i,j)/y(i,j)     
!	        ! PRINT *, 'transfer_to_y(index_2):',transfer_to_y(index_2)
!            END IF         
!        END DO
!        loan_to_y(1:sample_length,index) = loan_ts(1:sample_length,index)/y_ts(1:sample_length,index)
!        b_to_y(1:sample_length,index) = b_ts(1:sample_length,index)/y_ts(1:sample_length,index)
!        g_to_y(1:sample_length,index) = gov_spending/y_ts(1:sample_length,index)
!   END IF  
!END DO
!
!
!!PRINT *, 'Sample moments'
!DO j=1,counter_no_def
!    mean_log_output(j) = SUM(LOG(y_ts(1:sample_length,j)))/sample_length
!    mean_output(j) = SUM(y_ts(1:sample_length,j))/sample_length
!    mean_spread(j) = SUM(spread_ts(1:sample_length,j))/sample_length
!    mean_rr(j) = SUM(rr_ts(1:sample_length,j))/sample_length
!    std_log_y(j) = (COVARIANCE(LOG(y_ts(1:sample_length,j))-mean_log_output(j), &
!        LOG(y_ts(1:sample_length,j))-mean_log_output(j),sample_length))**0.5d+0 
!    std_output(j) = (COVARIANCE(y_ts(1:sample_length,j)-mean_output(j), &
!        y_ts(1:sample_length,j)-mean_output(j),sample_length))**0.5d+0 
!    std_spread(j) = (COVARIANCE(spread_ts(1:sample_length,j)-mean_spread(j), &
!        spread_ts(1:sample_length,j)-mean_spread(j),sample_length))**0.5d+0 
!    corr_spread_y(j) = COVARIANCE(spread_ts(1:sample_length,j), y_ts(1:sample_length,j),sample_length)   &
!        /(std_spread(j)*std_output(j))
!
!    mean_loan_to_y(j) = sum(loan_to_y(1:sample_length,j))/sample_length
!	mean_g_to_y(j) = sum(g_to_y(1:sample_length,j))/sample_length
!    mean_b_to_y(j) = sum(b_to_y(1:sample_length,j))/sample_length
!
!END DO
!
!!PRINT *, 'Compute moments'
!avg_loan_to_y = sum(mean_loan_to_y)/counter_no_def
!avg_transfer_to_y = sum(transfer_to_y)/counter_bc_no_def
!avg_std_log_y = sum(std_log_y)/counter_no_def
!avg_g_to_y = sum(mean_g_to_y)/counter_no_def
!avg_b_to_y = sum(mean_b_to_y)/counter_no_def
!avg_spread = sum(mean_spread)/counter_no_def
!avg_std_spread = sum(std_spread)/counter_no_def
!avg_corr_spread_y = sum(corr_spread_y)/counter_no_def
!avg_rr = sum(mean_rr)/counter_no_def
!
!PRINT *, '<< No exclusion for 50 consecutive periods >>'
!WRITE(*,*) 'avg loan to y', 100d+0 * avg_loan_to_y
!WRITE(*,*) 'avg bailout to y (conditional on BC)', 100d+0 * avg_transfer_to_y
!WRITE(*,*) 'Std Dev log(y)', 100d+0 * avg_std_log_y
!WRITE(*,*) 'avg g to y', 100d+0 * avg_g_to_y
...
!WRITE(*,*) 'corr(spread,y)', avg_corr_spread_y
!WRITE(*,*) 'avg loan rate (percent)', 100d+0 * avg_rr
!PRINT *, 'counter_bc_no_def', counter_bc_no_def
!
...
!DEALLOCATE (q_ts)
!DEALLOCATE (loan_ts)
!DEALLOCATE (transfer_ts)
!DEALLOCATE (spread_ts)
!DEALLOCATE (rr_ts)
!DEALLOCATE (loan_to_y)
!DEALLOCATE (g_to_y)
!
!DEALLOCATE (std_log_y)
!DEALLOCATE (mean_loan_to_y)
!DEALLOCATE (mean_g_to_y)
!DEALLOCATE (b_to_y)
!DEALLOCATE (transfer_to_y)
!DEALLOCATE (mean_spread)
!DEALLOCATE (std_spread)
!DEALLOCATE (mean_output)
!DEALLOCATE (mean_log_output)
!DEALLOCATE (std_output)
!DEALLOCATE (corr_spread_y)
!DEALLOCATE (mean_rr)
