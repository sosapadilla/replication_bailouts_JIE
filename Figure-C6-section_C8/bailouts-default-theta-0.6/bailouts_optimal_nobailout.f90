...
PARAMETER (sigma = 2d+0, pi_number = 3.1415926535897932d+0, lambda = 0d+0, zero = 0d+0, width = 1.5d+0,     &
    prob_excl_end = 0.6d+0, delta_bond = 1.0d+0, theta=1d+0, k=1d+0, mean_y = 0d+0, epsmin = 1.0d+0,        &
...
SUBROUTINE welfar
    ...
    IF ((transfer_num.EQ.1) .AND. (T_max_frac.LT.0.5d+0)) THEN
        def_nb = default_matrix
        bnext_nb = b_next_matrix
        bnext0_nb = b_next0_matrix
        labor_nb = n_matrix
        labor0_nb = labor0_matrix
        labor1_nb = labor1_matrix
        cons_nb = consumption_matrix
        cons0_nb = consumption0_matrix
        cons1_nb = consumption1_matrix
        bank_nb = cons_bank_matrix
        bank0_nb = cons_bank0_matrix
        bank1_nb = cons_bank1_matrix
    ELSE
