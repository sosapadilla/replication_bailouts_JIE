...
i_b_global, i_default_global, i_b_next,  i_b_nodil, iteration,  tax_num, transfer_num, transfer_num_use, epsilon_num
...
DOUBLE PRECISION :: sigma = 2d+0, b_inf, b_sup, y_inf, y_sup, eps_inf, eps_sup, std_y, &
    pi_number = 3.1415926535897932d+0, lambda = 0d+0, zero = 0d+0, width = 1.5d+0, &
    prob_excl_end = 0.5d+0, pi = 0.03d+0, coupon = 1d+0, delta_bond = 1.0d+0, theta = 1d+0, &
    tax_inf, tax_sup, k = 1d+0, mean_y = 0d+0, rw = 0.04d+0, transfer_inf, transfer_sup, &
    epsmin = 1.0d+0, epsmax = EXP(1.0d+0), bankcrisis_prob = 0.0181d+0, A_worst_case, indicator_lumpsum
...
DOUBLE PRECISION :: beta_bank = 0.96d+0, ghh_indicator = 1d+0, omega = 2.5d+0, sigma_c = 2d+0, &
    ...
    m = 0d+0, gamma_bank = 1d+0, phi_0 = 1d+0, phi_e = 0d+0, phi_y = 0d+0, phi_b = 0d+0
...
INTEGER :: ios, u
LOGICAL :: exists
NAMELIST /runtime_controls/ writeout, simulonly, maxiter, runwelf, indicator_external, indicator_bailout_default, &
    beta, AA, gov_spending, sig_e, T_max_frac, prob_excl_end, theta, omega, alpha, gamma_firm, pi, &
    phi_0, phi_e, phi_y, phi_b, transfer_num_use
...
CLOSE(u)
IF (ios /= 0) THEN
    WRITE(*,*) 'Warning: error reading runtime parameter file: ', TRIM(filename), ' IOSTAT=', ios
ELSE
...
first_mom, second_mom, std_pareto!, std_dev_eshock
...
ELOOP: DO i =1,epsilon_num-1
   ...
   ! PRINT *, "std_dev of transformed grid", std_dev_eshock
    ...
    ! Preserve pi from params_current.nml for sensitivity runs.
    ! The old calibration line below overwrote Low pi / High pi cases.
    PRINT *, "pi shock", pi
...
DO i_crisis_next = 1,2
   DO i_e_next = 1,epsilon_num
      DO i_y_next=1,y_num
         y_next = y_grid(i_y_next)
         m_next = beta_bank
         exp_m   = exp_m + m_next * coupon * &
         (1d+0 + r0_matrix(i_b, i_y_next, i_e_next, i_crisis_next)) *&
         trans_matrix(i_y, i_y_next)*petran(i_e_next)*prob_crisis(i_crisis_next)
...
wage, trans, guarantee, spread_ss, tr_y_elasticity, tr_y_multiplier !, welf_sim, wage_repayment, nn_repayment
...
WRITE(6,'(A,F20.10)') 'default_freq_conditional_bc=', def_prob_conditional*100.0d+0
