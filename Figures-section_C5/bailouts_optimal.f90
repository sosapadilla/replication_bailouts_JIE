...
INTEGER :: 	writeout = 0, simulonly = 0, maxiter = 100, runwelf = 1
DOUBLE PRECISION :: indicator_external = 0d+0, indicator_bailout_default = 0d+0,&
    indicator_phi_state_dep = 1d+0
...
INTEGER, PARAMETER	:: N_bita = 1, N_abar = 1, N_sigmae = 1, N_gov = 1, N_tmax = 1, &
    N_phi0 = 100, N_phie = 5, N_phiy = 5, N_phib = 5
...
rf_rate = 1.0d+0/beta_bank-1.0d+0, coupon = rf_rate/(1.0d+0 + rf_rate), mrate = 1.0d+0)! mrate = 0.70d+0)
...
ifcmdargs: IF (argtot > 0) THEN
    ...
    IF (N_phi0>1) THEN 
		CALL creategrid(phi0_grid, N_phi0, -0.8d+0, 0.0d+0, 0.2d+0, 1.2d+0, 1.0d+0)
		!PRINT *, 'phi0_grid:', phi0_grid
		phi_0 = phi0_grid(idx_vec(6))  
    END IF  

    IF (N_phie>1) THEN 
		CALL creategrid(phie_grid, N_phie, 0.00d+0, AA, 0.4d+0, 1.5d+0, 1.0d+0)
		!PRINT *, 'phie_grid:', phie_grid
		phi_e = phie_grid(idx_vec(7))  
    END IF  

    IF (N_phiy>1) THEN 
		CALL creategrid(phiy_grid, N_phiy, 0.0d+0, 2.0d+0, 2.0d+0, 1.0d+0, 1.0d+0)
		!PRINT *, 'phiy_grid:', phiy_grid
		phi_y = phiy_grid(idx_vec(8))  
    END IF  

    IF (N_phib>1) THEN 
		CALL creategrid(phib_grid, N_phib, 0.0d+0, 2.0d+0, 2.0d+0, 1.0d+0, 1.0d+0)
