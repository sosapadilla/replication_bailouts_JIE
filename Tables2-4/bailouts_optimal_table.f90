...
INTEGER :: 	writeout = 1, simulonly = 0, maxiter = 100, runwelf = 1, create_csv_tables = 1, workflow_mode = 2, csv_mode = 1
...
INTEGER, PARAMETER :: b_num=50, y_num=25, tax_num=25, transfer_num=50, epsilon_num=4
INTEGER :: eps_num, quad_num, nout, i_y_global, i_e_global, index_crisis_global, &
i_b_global, i_default_global, i_b_next,  i_b_nodil, iteration, transfer_num_active = 50
...
IF (transfer_num_active>1) THEN
    DO i=1,transfer_num_active
	    transfer_grid(i) = transfer_inf + (transfer_sup - transfer_inf) * (i-1) / (transfer_num_active - 1)
	    IF (writeout.EQ.1) THEN
...
ELOOP: DO i =1,epsilon_num-1
   ...
   ! PRINT *, "std_dev of transformed grid", std_dev_eshock
    ...
    if ( indicator_phi_state_dep > 0.50d+0 ) then
        ! phi_0_local  = AA*eshock(epsilon_num)*T_max_frac
        phi_0_local  = phi_0
...
PRINT *, 'workflow_mode = 1 => bailout only (Tables 2 and 3)'
PRINT *, 'workflow_mode = 2 => no-bailout + bailout comparison (Table 4 and welfare)'

CALL cpu_time(start_time)

CALL EXECUTE_COMMAND_LINE('mkdir -p nb_files')

IF (workflow_mode .EQ. 1) THEN
    CALL set_bailout_case_table23()
    CALL run_current_case(.FALSE.)
ELSE IF (workflow_mode .EQ. 2) THEN
    ! Correct Table 4 workflow:
    ! (1) solve NO-BAILOUT first with transfer_num_active=1 and T_max_frac=0.0d0
    ! (2) save those files into nb_files/
    ! (3) solve BAILOUT second with transfer_num_active=50 and T_max_frac=1.0d0
    ! No other model indicator is changed here.
    CALL EXECUTE_COMMAND_LINE('rm -rf nb_files')
    CALL EXECUTE_COMMAND_LINE('mkdir -p nb_files')

    PRINT *, '========================================'
    PRINT *, 'TABLE 4 STEP 1: NO-BAILOUT SOLVE'
    PRINT *, '  transfer_num_active = 1'
    PRINT *, '  T_max_frac          = 0.0d0'
    PRINT *, '========================================'
    CALL set_no_bailout_case()
    CALL run_current_case(.TRUE.)
    CALL copy_txt_outputs_to_nb_files()

    PRINT *, '========================================'
    PRINT *, 'TABLE 4 STEP 2: BAILOUT SOLVE'
    PRINT *, '  transfer_num_active = 50'
    PRINT *, '  T_max_frac          = 1.0d0'
    PRINT *, '========================================'
    CALL set_bailout_case_compare()
    CALL run_current_case(.FALSE.)
ELSE
    ERROR STOP 'workflow_mode must be 1 (bailout only) or 2 (comparison)'
END IF
...
SUBROUTINE set_no_bailout_case()
    ! No-bailout case for Table 4: ONLY the transfer grid and cap are changed.
    ! Keep all structural/model indicators unchanged.
    runwelf = 0
    create_csv_tables = 0
    csv_mode = 0
    writeout = 1
    simulonly = 0
    transfer_num_active = 1
    T_max_frac = 0.0d0
END SUBROUTINE set_no_bailout_case

SUBROUTINE set_bailout_case_table23()
    ! Baseline/bailout case for Tables 2 and 3.
    runwelf = 0
    create_csv_tables = 1
    csv_mode = 1
    writeout = 1
    simulonly = 0
    transfer_num_active = 50
    T_max_frac = 1.0d0
END SUBROUTINE set_bailout_case_table23

SUBROUTINE set_bailout_case_compare()
    ! Baseline/bailout case for Table 4. Welfare uses the no-bailout
    ! policy files already saved in nb_files/ from Step 1.
    runwelf = 1
    create_csv_tables = 1
    csv_mode = 2
    writeout = 1
    ...
    transfer_num_active = 50
    T_max_frac = 1.0d0
END SUBROUTINE set_bailout_case_compare

SUBROUTINE copy_txt_outputs_to_nb_files()
    IMPLICIT NONE
    PRINT *, 'Copying no-bailout benchmark files to nb_files/...'
    CALL EXECUTE_COMMAND_LINE('mkdir -p nb_files')
    CALL EXECUTE_COMMAND_LINE('cp -f graphs_default_dss.txt nb_files/')
    CALL EXECUTE_COMMAND_LINE('cp -f graphs_b_next.txt nb_files/')
    CALL EXECUTE_COMMAND_LINE('cp -f graphs_labor.txt nb_files/')
    CALL EXECUTE_COMMAND_LINE('cp -f graphs_consumption.txt nb_files/')
    CALL EXECUTE_COMMAND_LINE('cp -f graphs_cons_bank.txt nb_files/')
END SUBROUTINE copy_txt_outputs_to_nb_files

SUBROUTINE run_current_case(skip_results_out)
USE param
IMPLICIT NONE
LOGICAL, INTENT(IN) :: skip_results_out
INTEGER  i_b , i_y , index_opt_transfer, i_e, i_crisis
DOUBLE PRECISION :: consumption1,  labor1, r1, loan1, &
    cons_no_bc, labor_no_bc, rate_no_bc, tax_no_bc, cons_bc, labor_bc, rate_bc, tax_bc, bc
DOUBLE PRECISION, EXTERNAL :: u_fun, y_fun
EXTERNAL ::  compute_grid, ps_lp_and_default, optimize_lp
...
PRINT *, 'T_max_frac:', T_max_frac
PRINT *, 'mrate:', mrate
PRINT *, 'transfer_num_active:', transfer_num_active
...
IF (simulonly.EQ.0) THEN
    PRINT *, 'Solve model for current case...'
	CALL iterate
END IF
IF (runwelf.EQ.1) CALL welfar
PRINT *, 'Run simulations for current case...'
CALL simulate

IF (.NOT. skip_results_out) THEN
    PRINT *, 'Results printing...'
    501 FORMAT (I8, 39f15.10, I8, 6f15.10)
    OPEN (5,FILE='./results.out',FORM="FORMATTED")
...
wage, trans, guarantee, spread_ss, tr_y_elasticity, tr_y_multiplier, exposure_rate !, welf_sim, wage_repayment, nn_repayment
...
DOUBLE PRECISION :: welfare_gain_table4_pct
      ...
      IF (excl(i-1,j) ==2 ) THEN  !COUNTRY WAS EXCLUDED IN THE PREVIOUS PERIOD
         IF (random_matrix(i,j,4) < prob_excl_end) THEN  ! EXCLUSION ENDS THIS PERIOD
            ...
            if ( T_max(i_b_current, i_y_current, i_e_current) > zero ) then
                ...
                ! if ( index_opt_t_matrix(i_b_current, i_y_current, i_e_current) < transfer_num .or. &
                ! eshock(i_e_current) .le. zero) then
...
! Replicate sim_bailouts_new.m "pre-default samples" computation:
! mean(trans/y | bc_crisis==1) over last 25 periods of paths with no default in last 35 periods.
! MATLAB writes period_num-1=1500 rows starting from Fortran i=2, so:
!   last 25 MATLAB rows -> Fortran i = period_num-24 : period_num
!   no-default check window (last 35) -> Fortran i = period_num-34 : period_num
win_def_start = period_num - 34  ! first period of the 35-period no-default check window
win_use_start = period_num - 24  ! first period of the 25-period moment window
sum_trans_bc_predef = 0d+0
cnt_trans_bc_predef = 0
DO j = 1, sample_num
    IF (SUM(dd(win_def_start:period_num, j) - 1) < 1) THEN  ! no default in last 35 periods
...
welfare_gain_table4_pct = 100d+0 * avg_welf_b(i)
WRITE(*,*) 'welfare rel. to no-bailout (%)', welfare_gain_table4_pct
...
IF (create_csv_tables.EQ.1) THEN
PRINT *, '----------------------------------------'
PRINT *, 'Replication summary'
PRINT *, 'Default frequency (%)', 100d0*def_prob_unconditional
PRINT *, 'Mean spread (%)', 100d0*avg_spread
PRINT *, 'Debt/GDP (%)', 100d0*avg_b_to_y
PRINT *, '----------------------------------------'

IF (csv_mode .EQ. 1) THEN
    call write_table23_outputs(trim(output_dir), 'baseline model', &
         ...
         avg_rr, avg_rr_bc)
ELSE IF (csv_mode .EQ. 2) THEN
    ! Table 4 is written only after the bailout solve.  The no-bailout
    ! moments are read from nb_files/table4_nb_moments.txt, which was
    ! created during the first (no-bailout) solve in workflow_mode=2.
    call write_table4_outputs(trim(output_dir), 'baseline model', &
         def_prob_unconditional, def_prob_conditional, avg_spread, avg_std_spread, &
         avg_b_to_y, avg_rr, avg_spread_bc, avg_std_spread_bc, avg_b_to_y_bc, avg_rr_bc, &
         welfare_gain_table4_pct)
END IF
END IF

! During workflow_mode=2, the first solve is the no-bailout benchmark.
! Save its moments immediately, before the code switches to the bailout case.
IF (workflow_mode .EQ. 2 .AND. csv_mode .EQ. 0) THEN
    call write_table4_nb_moments('nb_files/table4_nb_moments.txt', &
         def_prob_unconditional, def_prob_conditional, avg_spread, avg_std_spread, &
         avg_b_to_y, avg_rr, avg_spread_bc, avg_std_spread_bc, avg_b_to_y_bc, avg_rr_bc)
END IF
...
SUBROUTINE write_table23_outputs(output_dir, model_label, &
     def_prob_unconditional, def_prob_conditional, def_banking_crisis, &
     avg_g_to_y, avg_transfer_to_y, avg_spread, avg_std_spread, avg_corr_spread_y, &
     avg_b_to_y, bailout_debt_corr, tr_multiplier, banking_sector_loss, &
     avg_spread_bc, avg_std_spread_bc, avg_b_to_y_bc, avg_bailout_to_y_uncond, &
  ...
  INTEGER :: u2, u3
  CHARACTER(LEN=512) :: f2, f3

  f2 = TRIM(output_dir)//'/table2_model_vs_data.csv'
  f3 = TRIM(output_dir)//'/table3_regime_comparison.csv'

call open_replace_local(u2, f2)
write(u2,'(A)') 'table,statistic,model_value'
call write_csv_row_blank_local(u2, 'Table 2', 'Default frequency (%)',             100d0*def_prob_unconditional)
call write_csv_row_blank_local(u2, 'Table 2', 'Banking crisis frequency (%)',      100d0*def_banking_crisis)
call write_csv_row_blank_local(u2, 'Table 2', 'Government spending/GDP (%)',       100d0*avg_g_to_y)
call write_csv_row_blank_local(u2, 'Table 2', 'Bailouts/GDP | banking crisis (%)', 100d0*avg_transfer_to_y)
call write_csv_row_blank_local(u2, 'Table 2', 'Sovereign spread mean (%)',         100d0*avg_spread)
call write_csv_row_blank_local(u2, 'Table 2', 'Sovereign spread sd (%)',           100d0*avg_std_spread)
call write_csv_row_blank_local(u2, 'Table 2', 'corr(spread y)',                    avg_corr_spread_y)
call write_csv_row_blank_local(u2, 'Table 2', 'Debt/GDP (%)',                      100d0*avg_b_to_y)
call write_csv_row_blank_local(u2, 'Table 2', 'corr(bailouts debt)',               bailout_debt_corr)
call write_csv_row_blank_local(u2, 'Table 2', 'Bailout-output multiplier',         tr_multiplier)
call write_csv_row_blank_local(u2, 'Table 2', 'Banking sector loss (%)',           100d0*banking_sector_loss)
close(u2)

  call open_replace_local(u3, f3)
  ...
  close(u3)
END SUBROUTINE write_table23_outputs

SUBROUTINE write_table4_nb_moments(file_name, &
     def_prob_unconditional, def_prob_conditional, avg_spread, avg_std_spread, &
     avg_b_to_y, avg_rr, avg_spread_bc, avg_std_spread_bc, avg_b_to_y_bc, avg_rr_bc)
  IMPLICIT NONE
  CHARACTER(LEN=*), INTENT(IN) :: file_name
  DOUBLE PRECISION, INTENT(IN) :: def_prob_unconditional, def_prob_conditional, avg_spread, avg_std_spread
  DOUBLE PRECISION, INTENT(IN) :: avg_b_to_y, avg_rr, avg_spread_bc, avg_std_spread_bc, avg_b_to_y_bc, avg_rr_bc
  INTEGER :: u

  call open_replace_local(u, file_name)
  write(u,'(ES24.16)') def_prob_unconditional
  write(u,'(ES24.16)') def_prob_conditional
  write(u,'(ES24.16)') avg_spread
  write(u,'(ES24.16)') avg_std_spread
  write(u,'(ES24.16)') avg_b_to_y
  write(u,'(ES24.16)') avg_rr
  write(u,'(ES24.16)') avg_spread_bc
  write(u,'(ES24.16)') avg_std_spread_bc
  write(u,'(ES24.16)') avg_b_to_y_bc
  write(u,'(ES24.16)') avg_rr_bc
  close(u)
END SUBROUTINE write_table4_nb_moments

SUBROUTINE write_table4_outputs(output_dir, model_label, &
     def_prob_unconditional, def_prob_conditional, avg_spread, avg_std_spread, &
     avg_b_to_y, avg_rr, avg_spread_bc, avg_std_spread_bc, avg_b_to_y_bc, avg_rr_bc, welfare_gain_pct)
  IMPLICIT NONE
  CHARACTER(LEN=*), INTENT(IN) :: output_dir, model_label
  DOUBLE PRECISION, INTENT(IN) :: def_prob_unconditional, def_prob_conditional, avg_spread, avg_std_spread
  DOUBLE PRECISION, INTENT(IN) :: avg_b_to_y, avg_rr, avg_spread_bc, avg_std_spread_bc, avg_b_to_y_bc, avg_rr_bc
  DOUBLE PRECISION, INTENT(IN) :: welfare_gain_pct
  INTEGER :: u4, unb, ios
  CHARACTER(LEN=512) :: f4, fnb
  DOUBLE PRECISION :: nb_def_u, nb_def_bc, nb_spread_u, nb_spread_sd_u, nb_debt_u, nb_rr_u
  DOUBLE PRECISION :: nb_spread_bc, nb_spread_sd_bc, nb_debt_bc, nb_rr_bc

  f4  = TRIM(output_dir)//'/table4_model_comparison.csv'
  fnb = 'nb_files/table4_nb_moments.txt'

  open(newunit=unb, file=trim(fnb), status='old', action='read', iostat=ios)
  if (ios /= 0) then
     print *, 'ERROR: could not read no-bailout Table 4 moments file: ', trim(fnb)
     print *, 'Run workflow_mode=2 so the code first solves the no-bailout case and saves nb_files/.'
     error stop
  end if
  read(unb,*) nb_def_u
  read(unb,*) nb_def_bc
  read(unb,*) nb_spread_u
  read(unb,*) nb_spread_sd_u
  read(unb,*) nb_debt_u
  read(unb,*) nb_rr_u
  read(unb,*) nb_spread_bc
  read(unb,*) nb_spread_sd_bc
  read(unb,*) nb_debt_bc
  read(unb,*) nb_rr_bc
  close(unb)

  call open_replace_local(u4, f4)

  ! Paper-style wide Table 4.  Units are percent.
  write(u4,'(A)') 'statistic,baseline_unconditional,baseline_banking_crisis,without_bailouts_unconditional,without_bailouts_banking_crisis'
  write(u4,'(A,",",F12.6,",",F12.6,",",F12.6,",",F12.6)') 'Default frequency', &
       100d0*def_prob_unconditional, 100d0*def_prob_conditional, 100d0*nb_def_u, 100d0*nb_def_bc
  write(u4,'(A,",",F12.6,",",F12.6,",",F12.6,",",F12.6)') 'Sovereign spread mean', &
       100d0*avg_spread, 100d0*avg_spread_bc, 100d0*nb_spread_u, 100d0*nb_spread_bc
  write(u4,'(A,",",F12.6,",",F12.6,",",F12.6,",",F12.6)') 'Sovereign spread sd', &
       100d0*avg_std_spread, 100d0*avg_std_spread_bc, 100d0*nb_spread_sd_u, 100d0*nb_spread_sd_bc
  write(u4,'(A,",",F12.6,",",F12.6,",",F12.6,",",F12.6)') 'Debt/GDP', &
       100d0*avg_b_to_y, 100d0*avg_b_to_y_bc, 100d0*nb_debt_u, 100d0*nb_debt_bc
  write(u4,'(A,",",F12.6,",",F12.6,",",F12.6,",",F12.6)') 'Mean lending rate', &
       100d0*avg_rr, 100d0*avg_rr_bc, 100d0*nb_rr_u, 100d0*nb_rr_bc
  write(u4,'(A,",",F12.6,",",A,",",A,",",A)') 'Welfare gain of bailouts', welfare_gain_pct, '', '', ''
  write(u4,'(A)') 'Units: percent. * denotes targeted moments.'
  close(u4)
END SUBROUTINE write_table4_outputs

SUBROUTINE open_replace_local(unit_no, file_name)
  IMPLICIT NONE
  ...
  error stop 'open_replace_local: could not find a free unit number.'
END SUBROUTINE open_replace_local
...
SUBROUTINE write_csv_row_blank_local(unit_no, table_name, stat_name, model_value)
  INTEGER, INTENT(IN) :: unit_no
  CHARACTER(*), INTENT(IN) :: table_name, stat_name
  DOUBLE PRECISION, INTENT(IN) :: model_value

  WRITE(unit_no,'(A,",",A,",",F20.10)') TRIM(table_name), TRIM(stat_name), model_value
END SUBROUTINE write_csv_row_blank_local

CHARACTER(LEN=64) FUNCTION real_to_text_local(x) RESULT(txt)
  IMPLICIT NONE
  DOUBLE PRECISION, INTENT(IN) :: x
  write(txt,'(F18.8)') x
  txt = adjustl(txt)
END FUNCTION real_to_text_local

CHARACTER(LEN=512) FUNCTION csv_escape_local(s) RESULT(out)
  IMPLICIT NONE
  ...
  out = '"' // TRIM(tmp) // '"'
END FUNCTION csv_escape_local

SUBROUTINE save_nb_files()
    IMPLICIT NONE

    CALL EXECUTE_COMMAND_LINE('mkdir -p nb_files')
    CALL EXECUTE_COMMAND_LINE('rm -f nb_files/graphs_default_dss.txt')
    CALL EXECUTE_COMMAND_LINE('rm -f nb_files/graphs_b_next.txt')
    CALL EXECUTE_COMMAND_LINE('rm -f nb_files/graphs_labor.txt')
    CALL EXECUTE_COMMAND_LINE('rm -f nb_files/graphs_consumption.txt')
    CALL EXECUTE_COMMAND_LINE('rm -f nb_files/graphs_cons_bank.txt')
