...
INTEGER :: writeout = 1, simulonly = 0, maxiter = 100, runwelf = 1
...
PARAMETER (b_num= 50, y_num =25, tax_num=25, transfer_num = 50, epsilon_num = 4)
...
DOUBLE PRECISION :: sigma, b_inf, b_sup, y_inf, y_sup, eps_inf, eps_sup, std_y, pi_number, lambda,  &
    zero, width, delta_bond, tax_inf, tax_sup, k, mean_y, rw,     &
...
PARAMETER (sigma = 2d+0, pi_number = 3.1415926535897932d+0, lambda = 0d+0, zero = 0d+0, width = 1.5d+0,     &
    delta_bond = 1.0d+0, k=1d+0, mean_y = 0d+0, epsmin = 1.0d+0,        &
    epsmax = EXP(1.0d+0), bankcrisis_prob = 0.0181d+0, rw = 0.04d+0,&
    damp_v= 0.5d+0, damp_q = 0.5d+0)
DOUBLE PRECISION :: prob_excl_end = 0.5d+0, theta = 1.0d+0, pi = 0.0d+0
DOUBLE PRECISION :: phi_0 = 1.0d+0, phi_e = 0.0d+0, phi_y = 0.0d+0, phi_b = 0.0d+0
INTEGER :: transfer_num_use = transfer_num
...
DOUBLE PRECISION :: beta_bank = 0.96d+0, ghh_indicator = 1d+0, omega = 2.5d+0, sigma_c = 2d+0, &
    ...
    m_NX = 0d+0, gamma_bank = 1d+0, rf_rate, mrate   = 0.75d+0, coupon
...
def_banking_crisis, avg_ytm_bond, avg_ytm_free, avg_spread_alt

CONTAINS 

SUBROUTINE initialize_runtime_parameters()
IMPLICIT NONE
rf_rate = 1.0d+0/beta_bank - 1.0d+0
coupon  = rf_rate/(1.0d+0 + rf_rate)
END SUBROUTINE initialize_runtime_parameters

SUBROUTINE read_runtime_params(filename)
IMPLICIT NONE
CHARACTER(LEN=*), INTENT(IN) :: filename
INTEGER :: ios, u
LOGICAL :: exists
NAMELIST /runtime_controls/ writeout, simulonly, maxiter, runwelf, &
    indicator_external, indicator_bailout_default, &
    beta, AA, gov_spending, sig_e, T_max_frac, &
    prob_excl_end, theta, omega, alpha, gamma_firm, pi, &
    phi_0, phi_e, phi_y, phi_b, transfer_num_use, mrate
INQUIRE(FILE=filename, EXIST=exists)
IF (.NOT. exists) RETURN
u = 91
OPEN(u, FILE=filename, STATUS='old', ACTION='read', IOSTAT=ios)
IF (ios /= 0) THEN
    WRITE(*,*) 'Warning: could not open runtime parameter file: ', TRIM(filename)
    RETURN
END IF
READ(u, NML=runtime_controls, IOSTAT=ios)
CLOSE(u)
IF (ios /= 0) THEN
    WRITE(*,*) 'Warning: error reading NML file: ', TRIM(filename), ' IOSTAT=', ios
ELSE
    CALL initialize_runtime_parameters()
    WRITE(*,*) 'Loaded runtime parameters from ', TRIM(filename)
END IF
END SUBROUTINE read_runtime_params
...
!! Set up the debt grid, with three segments:
!! 1. Low debt: [b_inf, b_int1]
!
!step = (b_int1 - b_inf) / real(Nb_low, 8)
!do i = 1, Nb_low
!    grid_low(i) = b_inf + step * real(i - 1, 8)
!end do
!
!! 2. Mid debt: [b_int1, b_int2]
!step = (b_int2 - b_int1) / real(Nb_mid, 8)
!do i = 1, Nb_mid
!    grid_mid(i) = b_int1 + step * real(i - 1, 8)
!end do
!
!! 3. High debt: [b_int2, b_sup]
!step = (b_sup - b_int2) / real(Nb_high, 8)
!do i = 1, Nb_high
!    grid_high(i) = b_int2 + step * real(i - 1, 8)
!end do
!
!! Concatenate into full debt_grid
!  idx = 0
!  do i = 1, Nb_low
!     idx = idx + 1
!     b_grid(idx) = grid_low(i)
!  end do
!  do i = 1, Nb_mid
!     idx = idx + 1
!     b_grid(idx) = grid_mid(i)
!  end do
!  do i = 1, Nb_high
!     idx = idx + 1
!     b_grid(idx) = grid_high(i)
!  end do
...
DOUBLE PRECISION function loan_supply_fun(b)
    USE param
    IMPLICIT NONE
    DOUBLE PRECISION, INTENT (IN) :: b
    DOUBLE PRECISION :: aux, d

    d = default_grid(i_default_global)

    if (indicator_bailout_default > 0.5d+0) then    
        aux = d*(A_worst_case + transfer_global) + &
            (1.0d+0 - d) * (b *(mrate + (1.0d+0 - mrate) * (coupon + q_vector(i_b_next, i_y_global))) +&
            A_worst_case + transfer_global)         
    else
        aux = d*(A_worst_case) + &
            (1.0d+0 - d) * (b *(mrate + (1.0d+0 - mrate) * (coupon + q_vector(i_b_next, i_y_global))) +&
            A_worst_case + transfer_global)                 
...
DOUBLE PRECISION function r_fun(n)
    ...
    denominator = loan_supply_fun(b_initial)
...
! ELSE
   ...
   ! This wage bill (w*n) comes from the "loan supply = loan demand" condition. It's therefore dependant on A_worst_case, and the "promised transfer"
    loan = loan_supply_fun(b_initial)
    ...
    IF (rr==r_min) THEN ! meaning the intersection of demand an supply happens at the horizontal part of the demand and:
        ...
        IF (exit_flag<0.5d+0) THEN
            ...
            labor=(EXP(y_initial)*alpha*k**(1d+0-alpha)*&
...
SUBROUTINE ps_repay_full(consumption,labor,loan, rate)
    ...
    DOUBLE PRECISION, EXTERNAL :: r_fun, y_fun, q_fun, loan_supply_fun
    ...
    debt_part = b_initial*(mrate + (1d+0-mrate)*coupon) -&
        (b_grid(i_b_next)-(1d+0-mrate)*b_initial) * q_vector(i_b_next,i_y_global)
...
DOUBLE PRECISION function q_fun_OLD(i_b, i_y)
    USE param
    IMPLICIT NONE
    ...
    DO i_e_next = 1,epsilon_num
        DO i_y_next=1,y_num
            ...
            DO i_crisis_next = 1,2
                exp_inner = exp_inner + & 
                ((1d+0 + r_matrix(i_b, i_y_next, i_e_next, i_crisis_next))*(mrate + (1d+0-mrate)*(coupon + &
                q_matrix_nodef(b_next0_matrix(i_b,i_y_next,i_e_next,i_crisis_next),i_y_next,i_e_next, i_crisis_next))))*&
                prob_crisis(i_crisis_next)
            end do
            exp_m = exp_m +  (1d+0 - default_grid(default_decision(i_b, i_y_next, i_e_next)))* &
...
DOUBLE PRECISION function rloan_next(i_b, i_y)
    USE param
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i_b, i_y
    DOUBLE PRECISION :: y_next, m_next, exp_m, exp_q, prob_crisis(2)
    ...
    DO i_crisis_next = 1,2
        DO i_e_next = 1,epsilon_num
            DO i_y_next=1,y_num
                exp_m = exp_m +(1d+0 + r_matrix(i_b, i_y_next, i_e_next,i_crisis_next))*&
                trans_matrix(i_y, i_y_next)*petran(i_e_next)*prob_crisis(i_crisis_next)
...
! Not used right now, but could be useful in the future.
...
SUBROUTINE solve_tax_full(tax_opt, exit_flag)
    ...
    DOUBLE PRECISION :: nn, rev_candidate, tao, tax_base,&
    rev_needed, exit_flag, tax_opt, ERRABS, left, right, dif_left, dif_right, tax_max,  tax_min, d, convergence ,&
    transfer_effective, bc, debt_part
   ...
   DO j=1,2
      ...
      DO i_b=1,b_num
         ...
         cons_banker= transfer_effective + ll*rr + b_initial * (mrate + (1d+0-mrate)*coupon) - &
            q_vector(i_b_next,i_y_global) * (b_grid(i_b_next) - (1d+0-mrate)*b_initial)
...
output_no_transfer, output_opt_transfer, dev_b
...
v_matrix_new, q_matrix_nodef_new, dev_q_matrix, w_matrix_new, vw_matrix_new, b_next0_matrix_new
      ...
      DO i_y = 1,y_num
         ...
         DO i_b = 1,b_num 
            ...
            DO i_crisis=1,2
               ...
               b_next0_matrix_new(i_b, i_y,i_e,i_crisis) = i_b_next_local(i_crisis)
               ...
               cons_banker = transfer0_matrix(i_b, i_y,i_e,i_crisis) + b_initial * (mrate + (1d+0-mrate)*coupon) + &
               loan0_matrix(i_b, i_y,i_e,i_crisis) * r0_matrix(i_b, i_y,i_e,i_crisis) - &
               q_matrix_nodef_new(i_b, i_y, i_e, i_crisis) * (b_grid(i_b_next) - (1d+0-mrate)*b_initial)
               ...
               dev_b = MAX(dev_b, ABS(b_next0_matrix_new(i_b, i_y, i_e, i_crisis) - b_next0_matrix(i_b, i_y, i_e, i_crisis)))
...
CALL read_runtime_params('params_current.nml')
...
IF (indicator_external < 0.5d+0) THEN
    ...
    b_next0_matrix = 1 ! Telling the code that there is no 
    q_matrix_nodef = 0d+0
    q_vector = 0d+0
      ...
      DO i_y = 1,y_num
         ...
         CALL optimize_lp(index_opt_transfer, cons_no_bc, labor_no_bc, rate_no_bc, tax_no_bc, cons_bc, labor_bc, rate_bc, tax_bc)
            ...
            DO i_crisis=1,2
               ...
               labor0_matrix(i_b, i_y,i_e,i_crisis))
               w0_matrix(i_b,i_y,i_e,i_crisis)  = transfer0_matrix(i_b, i_y,i_e,i_crisis) + b_initial *(mrate + (1d+0-mrate)*coupon) + &
               loan0_matrix(i_b, i_y,i_e,i_crisis) * r0_matrix(i_b, i_y,i_e,i_crisis)
...
501 FORMAT (I8, 35f15.10, I8, 4f15.10)
...
INTEGER, DIMENSION (period_num,sample_num) :: dd, excl, bc_crisis, y_dev, y_trend !, bad_epsilon
...
wage, trans, guarantee, spread_ss, tr_y_elasticity, tr_y_multiplier, ytm_bond, ytm_free, spread_alt !, welf_sim, wage_repayment, nn_repayment
!DOUBLE PRECISION, DIMENSION (:,:), ALLOCATABLE  :: y_ts, q_ts, loan_ts, b_ts, transfer_ts, loan_to_y,g_to_y, &
!    b_to_y, spread_ts, rr_ts
!DOUBLE PRECISION, DIMENSION (:), ALLOCATABLE  :: std_log_y, mean_loan_to_y, mean_b_to_y, transfer_to_y, mean_g_to_y, &
!    mean_log_output, mean_output, mean_spread, std_spread, std_output, corr_spread_y, mean_rr
DOUBLE PRECISION, EXTERNAL :: q_fun, objective_function, r_fun, q_fun_nodef, rloan_next
   ...
   DO i=2,period_num
      ...
      IF (excl(i-1,j) ==2 ) THEN  !COUNTRY WAS EXCLUDED IN THE PREVIOUS PERIOD
         IF (random_matrix(i,j,4) < prob_excl_end) THEN  ! EXCLUSION ENDS THIS PERIOD
            ...
            ytm_free(i,j) =rloan_next(i_b_next,i_y_current)*(1d+0+rf_rate)-1d+0
      ...
      ELSE ! THE COUNTRY WAS NOT EXCLUDED THE PREVIOUS PERIOD
         ...
         IF (default_decision(i_b_current, i_y_current, i_e_current) > 1) THEN  !COUNTRY DEFAULTS
            ...
            !nn_repayment(i,j)=labor0_matrix(i_b_current, i_y_current, i_e_current, i_crisis_current)
            !wage_repayment(i,j)=EXP(z(i,j))*alpha*nn_repayment(i,j)**(alpha-1d+0)/(1d+0 + gamma_firm*   &
            !    r0_matrix(i_b_current, i_y_current, i_e_current, i_crisis_current) )
            tax_repayment(i,j) = tax0_matrix(i_b_current, i_y_current, i_e_current, i_crisis_current)
...
!DO j=1,sample_num
!    CALL hpfilt(LOG(y(:,j)), y_trend(:,j), period_num, 100)  !(y,t,n,s)
!END DO
!
!y_dev = LOG(y) - y_trend
!avg_log_y = SUM( y_dev(period_start:period_num,:) ,excl(period_start:period_num,:).EQ.1 )/  &
!    COUNT(excl(period_start:period_num,:).EQ.1)
!WRITE(*,*) 'avg Dev log(y) HP-filtered', 100d+0 * avg_log_y
!avg_std_log_y = &
!    ( SUM( y_dev(period_start:period_num,:)**2,excl(period_start:period_num,:).EQ.1 )/  &
!    COUNT(excl(period_start:period_num,:).EQ.1) )**0.5d+0
!WRITE(*,*) 'Std Dev log(y) HP-filtered', 100d+0 * avg_std_log_y
...
! avg_welf = SUM(welf_sim(period_start:period_num,:), excl(period_start:period_num,:).EQ.1) &
!     /COUNT(excl(period_start:period_num,:).EQ.1)
! WRITE(*,*) 'welfare gain rel. to no bailout (percent)', 100d+0 * avg_welf

!avg_cons = SUM(c(period_start:period_num,:), excl(period_start:period_num,:).EQ.1)   &
!    /COUNT(MASK = excl(period_start:period_num,:).EQ.1)
!PRINT *, 'Mean consumption:', avg_cons
!PRINT *, 'St Dev cons (percent):', 100d+0*  &
!    SUM((c(period_start:period_num,:)-avg_cons)**2, excl(period_start:period_num,:).EQ.1) &
...
! Write summary for MATLAB runner
OPEN(6, FILE='summary_results.txt', FORM='FORMATTED', STATUS='replace')
WRITE(6,'(A,F20.10)') 'default_freq_unconditional=', def_prob_unconditional*100.0d+0
WRITE(6,'(A,F20.10)') 'spread_mean=', avg_spread*100.0d+0
WRITE(6,'(A,F20.10)') 'spread_sd=', avg_std_spread*100.0d+0
WRITE(6,'(A,F20.10)') 'corr_spread_y=', avg_corr_spread_y
WRITE(6,'(A,F20.10)') 'debt_gdp=', avg_b_to_y*100.0d+0
WRITE(6,'(A,F20.10)') 'mean_lending_rate=', avg_rr*100.0d+0
WRITE(6,'(A,F20.10)') 'welfare_gain=', avg_welf_b(i)*100.0d+0
CLOSE(6)
